...
def test_cartitem_create(api_client, user):
    ...
    api_client.force_authenticate(user=user)
    data = {'cart': str(cart.id), 'product': str(product.id), 'quantity': 1, 'price': '10.00'}
...
def test_cartitem_create_invalid_product(api_client, user):
    ...
    url = reverse('cart:cartitem-list-create-top')
    api_client.force_authenticate(user=user)
    data = {'cart': str(cart.id), 'product': '00000000-0000-0000-0000-000000000000', 'quantity': 1, 'price': '10.00'}
    response = api_client.post(url, data)
